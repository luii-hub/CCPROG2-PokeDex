...
/*  Things to Change after  100% Finish of MP
    ...
    New bug/s:
    - Main Menu loops "invalid input" (n) times when user inputs multiple (n) characters
...
{   
    ...
    do{ /*Research Tasks Menu */
        ...
        printf("Add-Ons:\n[5] Add Additional Research Task\n[6] Show Additional Research Task\n[7] Update Research Task Add-On\n[8] Show Research Task Add-on Per Pokemon\n[9] Exit\n\n");
        printf("Console: ");
        scanf("%d", &nChoice); printf("\n");
        while(nChoice <= 0 || nChoice > 9)
        {
...
void addResearchTask(struct details entry[], int *storedEntries, int *addOnCounter, char researchAddOns[][21]){
    ...
    if(*storedEntries == 0)
    {
        ...
        if(*addOnCounter > 4){
            printf("Error: You have reached the Maximum Number of Additional Tasks\n");
        }
        else{
            ...
            if(cChoice == 'n'){
                ...
                do{
                    printf("Note: Research Task must not exceed more than 20 Characters!\nResearch Task Description: ");
                    scanf(" %[^\n]s", modify20); printf("\n");
                ...
                printf("Successfully added a Research Task\n");    
...
void showAdditionalTasks(struct details entry[], int *storedEntries, int* addOnCounter, char researchAddOns[][21]){
    ...
    if(*storedEntries == 0){
        ...
        if(*addOnCounter == 0){
            printf("No Research Task Add-Ons Added\n");
        }
        else{
            printf("Additional Task Types:\n");
...
void showAddOnsPerPkmn(struct details entry[], int *storedEntries, int* addOnCounter, char researchAddOns[][21]){
    ...
    if(*storedEntries == 0){
        ...
        if(*addOnCounter == 0){
            ...
            printf("List of Pokemon & Research Task Add-Ons\n");
            for(i = 0; i < *storedEntries; i++){
                printf("Pokemon: %s\n", entry[i].name20);
