...
/*  Things to Change after  100% Finish of MP
    ...
    - Return Menu not working *BUG FIXED*
...
void pokemonRank(struct details entry[], int *storedEntries)
{
    ...
    // int i, j, max;
    // int temp;
    // int totalTasks[*storedEntries], ranked[maxRank];
    // int k, ctr, entryCount;
    ...
    else
    {
        ...
        for(i = 0; i < *storedEntries; i++)
        {
            totalTasks[i] = entry[i].seen + entry[i].defeated;
        }
        ...
        for(i = 0; i < *storedEntries - 1; i++)
        {
            ...
            {
                if(totalTasks[max] < totalTasks[j])
                {
                    max = j;
                }
            }
        ...
        for(i = 0; i < maxRank; i++) // rank
        {
            ctr = 0;
            entryCount = 0;
            for(j = 0; j < *storedEntries; j++) // entry
            {
                if(totalTasks[i] == (entry[j].seen + entry[j].defeated) && ctr == 0)
                {
                    // printf("TOP %d\nPokemon Name: %s\nPokemon Type: %s\nDescription: %s\nSeen: %d\nDefeated: %d\n\n", (i+1), entry[j].name20, entry[j].type20, entry[j].description50, entry[j].seen, entry[j].defeated);
                    for(k = 0; k < maxRank; k++)
                    {
                        if(ranked[k] == j)
                        {
        ...
        for(i = 0; i < maxRank; i++)
        {
            printf("TOP %d\nPokemon Name: %s\nPokemon Type: %s\nDescription: %s\nSeen: %d\nDefeated: %d\n\n", (i+1), entry[ranked[i]].name20, entry[ranked[i]].type20, entry[ranked[i]].description50, entry[ranked[i]].seen, entry[ranked[i]].defeated);
        }
        ...
        */
        ...
        for(i = 0; i < *storedEntries; i++)
        {
            ...
            // printf("Index: %d   ", pokemon[i].index + 1); // TEST
            // printf("Total: %d\n", pokemon[i].total); // TEST
        }
        // printf("\n"); // TEST
        ...
        for(i = 0; i < *storedEntries - 1; i++)
        {
            ...
            // if(i != max)
            // {
            //     temp = pokemon[i].total;
            //     pokemon[i].total = pokemon[max].total;
            //     pokemon[max].total = temp;
            // }
        ...
        // while(i<*storedEntries) 
        // {
        //     printf("Index: %d   ", pokemon[i].index + 1);
        //     printf("Total: %d\n", pokemon[i].total);
        //     i++;
        // }
        ...
        for(i = 0; i < maxRank; i++)
        {
            printf("TOP %d\nPokemon Name: %s\nPokemon Type: %s\nDescription: %s\nSeen: %d\nDefeated: %d\n\n", (i+1), entry[pokemon[i].index].name20, entry[pokemon[i].index].type20, entry[pokemon[i].index].description50, entry[pokemon[i].index].seen, entry[pokemon[i].index].defeated);
        }
    }
}